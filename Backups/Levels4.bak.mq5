...
input bool     InpEnableTrading              = false;    // MASTER switch: false = analysis only, true = EA can place trades
input double   InpRiskPercent                = 1.0;      // Risk % per trade (safe start: 0.5 to 1.0)
input double   InpFixedLotIfRiskOff          = 0.10;     // Fallback lot size if risk sizing cannot be calculated
input int      InpStopLossPoints             = 300;      // Stop Loss distance (points)
input int      InpTakeProfitPoints           = 600;      // Take Profit distance (points)
input double   InpMinLot                     = 0.01;     // Minimum lot allowed by your rules
input double   InpMaxLot                     = 100.0;    // Maximum lot allowed by your rules
input int      InpSlippagePoints             = 20;       // Max entry slippage in points
input bool     InpAllowBuy                   = true;     // Allow BUY trades
input bool     InpAllowSell                  = true;     // Allow SELL trades
input ulong    InpMagicNumber                = 550001;   // Unique ID for this EA's trades
input int      InpMaxSpreadPoints            = 30;       // Do not trade if spread is above this value
input double   InpHardProfitTargetMoney      = 100.0;    // DAILY profit lock in account currency (example: 100 = lock after +100)
input bool     InpClosePositionsOnHardLock   = true;     // When profit lock hits: close open managed trades immediately
input bool     InpRiskScopeMagicOnly         = true;     // Manage only this EA's magic number trades
input bool     InpRiskScopeSymbolOnly        = true;     // Manage only current chart symbol trades
input bool     InpEnableTrailing             = true;     // Turn trailing stop ON/OFF
input int      InpTrailingStartPoints        = 200;      // Start trailing only after this much profit (points)
input int      InpTrailingDistancePoints     = 150;      // Trailing SL distance from current price (points)
input int      InpTrailingStepPoints         = 20;       // Update trailing SL only if it improves by at least this many points
